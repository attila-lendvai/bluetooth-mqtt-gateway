  ...
  (let (
    ...
    (c-with ((device-address bluez:bdaddr-t))
      ...
      (let* ((device-id (c-fun/rc bluez:hci-devid mac))
        ...
        (unwind-protect
             (progn
               (format t "Socket is: ~S, mac is: ~S, device name is: ~S~%" socket mac (bluez:hci-device-name device-id))
               (bluez:hci/reset-device socket device-id)
               (format t "Device reset successful~%")
          ...
          (c-fun/rc bluez:hci-close-dev socket)))))
  (format t "Exiting~%"))
